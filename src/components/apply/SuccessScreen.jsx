...
const nextSteps = [
  { label:"Application Received",  desc:"We've received your application and it's now under review" },
  { label:"Initial Screening",     desc:"Our recruitment team will review your assessment and profile" },
  { label:"Interview",             desc:"Shortlisted candidates will be contacted for a scheduled interview" },
  { label:"Final Decision",        desc:"You'll receive an email with the outcome of your application" },
];
  ...
  return (
    <div className="max-w-[640px] mx-auto py-6">
      ...
      <div className="text-center mb-8">
        <h1 className="font-jakarta font-bold text-black text-2xl mb-3">Application Submitted</h1>
        ...
        <p className="text-sm text-zinc-400 max-w-md mx-auto">
          Thank you for applying. We'll review your submission and get back to you within 3-5 business days
        </p>
      </div>
      ...
      <div className="border border-border rounded-2xl px-6 py-5 mb-8 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1">Application Reference</p>
      ...
      {/* What happens next */}
      <div className="mb-8">
        <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-5">What Happens Next</p>
            ...
            <div key={step.label} className="flex gap-4">
              ...
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all
                  ${i === 0 ? "gradient-bg border-transparent"
                  : i === 1 ? "gradient-bg border-transparent"
                  : "border-zinc-200 bg-white"}`}>
                  {i === 0 ? (
                    <Check size={13} weight="bold" className="text-white"/>
                  ) : (
                    <span className={`text-xs font-bold font-jakarta ${i <= 1 ? "text-white" : "text-zinc-400"}`}>{i+1}</span>
                  )}
                </div>
                {i < nextSteps.length - 1 && (
                  <div className={`w-px flex-1 my-1 ${i === 0 ? "bg-primary" : "bg-zinc-200"}`}/>
                )}
              </div>
              {/* Content */}
              <div className="pb-5">
