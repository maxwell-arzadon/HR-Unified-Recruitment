        ...
        return (
          <div key={stage} className="flex items-center flex-1 last:flex-none">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs font-semibold tracking-wide uppercase ${isCurrent || isDone ? "text-primary" : "text-muted"}`}
              >
                {stage}
              </span>
            </div>
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      ...
      <div className="bg-bg border border-border rounded-xl p-4">
        ...
        <span
          className={`font-jakarta font-bold text-2xl ${assessment.essay === "Passed" ? "text-success" : "text-primary"}`}
        >
          {assessment.essay}
        </span>
      </div>
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden">
      {rows.map(({ label, value }, i) => (
        <div
          key={label}
          className={`grid grid-cols-2 px-4 py-3 ${i < rows.length - 1 ? "border-b border-border" : ""}`}
        >
...
// ─── New Applicant Content ────────────────────────────────────────
function NewApplicantContent({ applicant }) {
  // Tab logic: "New" status shows Schedule, all others show Update Status
  const isNew = applicant.status === "New";
  const TABS = ["Overview", "Assessment", isNew ? "Schedule" : "Update Status"];
  ...
  const appDetailRows = [
    { label: "Ref Number", value: applicant.ref },
  ...
  // Status options excluding "All Status" and "New"
  const statusOptions = STATUSES.filter(
    (s) => s !== "All Status" && s !== "New",
  );
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-6">
        ...
        {/* Schedule — only for "New" status applicants */}
        {activeTab === "Schedule" && (
          <>
            <div>
              <SectionLabel>Hiring Stage</SectionLabel>
        ...
        {/* Update Status — for Interview, Orientation, and other non-New statuses */}
        {activeTab === "Update Status" && (
          <>
            ...
            <div className="flex flex-col gap-4">
              ...
              <button className="w-full gradient-bg text-white font-semibold font-jakarta text-sm py-3.5 rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">
                {applicant.ref} • {applicant.position} • {applicant.jobType}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
