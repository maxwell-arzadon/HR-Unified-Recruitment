...
import {
  EnvelopeSimple, Headset, Megaphone, Monitor,
  Code, MagnifyingGlass, Globe, UsersThree,
  Briefcase, MapPin, Users, Clock, Check,
...
export default function JobDetail() {
  ...
  return (
    ...
    <div className="max-w-[1440px] mx-auto px-6 sm:px-10 md:px-16 lg:px-[80px] py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="flex-1 bg-white border border-border rounded-2xl p-8 min-w-0">
            ...
            <div className="flex items-start gap-4 mb-6">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 ${iconColors[job.type]}`}>
                {iconMap[job.icon]}
              </div>
              <div>
                <h1 className="font-jakarta font-bold text-black text-2xl mb-2">{job.title}</h1>
            ...
            <div className="text-sm text-zinc-500 leading-relaxed">
              ...
              {activeTab === "FAQ" && (
                <div className="flex flex-col gap-6">
                  {job.faq.map((f, i) => (
                    <div key={i}>
                      <h4 className="font-jakarta font-bold text-black mb-1">{f.q}</h4>
                      <p>{f.a}</p>
                    </div>
                  ))}
                </div>
          ...
          <div className="w-full lg:w-72 flex flex-col gap-4 flex-shrink-0">
            ...
            <div className="bg-white border border-border rounded-2xl p-6">
              <h3 className="font-jakarta font-bold text-black text-base mb-5">Job Overview</h3>
              <div className="flex flex-col gap-5">
                {[
                  { icon: <Briefcase size={20} className="text-primary" />, label: "JOB TYPE",    value: job.type     },
                  { icon: <MapPin    size={20} className="text-primary" />, label: "LOCATION",    value: job.location },
                  { icon: <Users     size={20} className="text-primary" />, label: "OPEN SLOTS",  value: `${job.slots} positions` },
                  { icon: <Clock     size={20} className="text-primary" />, label: "SCHEDULE",    value: job.schedule },
            ...
            <div className="bg-red-50 border border-red-100 rounded-2xl p-6">
              <h3 className="font-jakarta font-bold text-black text-base mb-2">Ready to apply?</h3>
              <p className="text-sm text-zinc-500 mb-5">
                Takes about 15–20 min. Have your personal details and SSS number ready.
              </p>
              <button
      ...
      {showModal && <BeforeYouApply onClose={() => setShowModal(false)} jobId={job.id} />}
    </div>
  );
}